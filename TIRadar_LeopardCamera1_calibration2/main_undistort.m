...
% 畸变系数
%RadialDistortion = [-0.308448929716604, 0.0985559783424981]; % k1, k2


%% 加载特征点
data_xyz=readmatrix("./data/OCULiiRadar_points.xlsx", "Range", [1 1]);
load ./runs_equivalent_undistort/imagePoints.mat
data(:, 1:3)=data_xyz(:, 1:3);
save ./runs_equivalent_undistort/allPoints data

%% 自动+手动剔除不好的点
data = data(data_xyz(:,4)==1,:);
data = data(sum(isnan(data),2)==0, :);
save ./runs_equivalent_undistort/calibPoints data
%% 
...
Bx=A\Hx;
save ./runs_equivalent_undistort/Hx Hx;%坐标转移矩阵存储。（输出）
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
