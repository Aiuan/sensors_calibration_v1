...
%A = [1974.49484146670,0,0;0,1976.17014138763,0;1797.40706319378,1001.28552420718,1]'; % intrinsic matrix of LeopardCamera 3517x2004 undistort
A = [1976.27129878769,0,0;0,1977.80114435384,0;1798.25228491297,1000.96808764067,1]'; % intrinsic matrix of LeopardCamera 3517x1700 undistort

%% 加载特征点
data_xyz=readmatrix("./data/TIRadar_points.xlsx", "Range", [1 1]);
load ./runs_undistort_cut/imagePoints.mat
data(:, 1:3)=data_xyz(:, 1:3);
save ./runs_undistort_cut/allPoints data

%% 自动+手动剔除不好的点
data = data(data_xyz(:,4)==1,:);
data = data(sum(isnan(data),2)==0, :);
save ./runs_undistort_cut/calibPoints data
%% 
...
Bx=A\Hx;
save ./runs_undistort_cut/Hx Hx;%坐标转移矩阵存储。（输出）
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
